...
- `scripts/multiagent_files.py` - shared run-folder and message helper.
...
`scripts/multiagent_files.py` is **not** copied into any platform's install root. All platforms invoke it from the canonical repo path. Update once, used by all.
...
- Keep a skill-installer or skill-search capability assigned to every custom agent so agents can search for missing skills.
- Role guidance references skills by *category* ("a context-maintenance skill (if available)"), not by concrete skill name. Do not hardcode specific skill names into shipped role files — that leaks personal setup. Concrete-skill examples belong in `examples/personal-profiles/`.
...
- Codex skill assignments: local skill paths belong in the generated `codex-agents/*.toml` (gitignored). Do not edit paths in `codex-agents/templates/*.toml`; edit the install script logic instead. Claude Code and Antigravity auto-discover skills from `~/.claude/skills/` and `~/.gemini/config/skills/` — no allowlist to maintain there.
